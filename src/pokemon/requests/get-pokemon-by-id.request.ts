...
} from '../queries/get-pokemon-by-id.query.js';

@Controller('pokemon')
export class GetPokemonByIdRequest
  implements Pick<PokedexControllerMethods, 'getPokemonById'>
{
  constructor(private readonly handler: GetPokemonByIdQueryHandler) {}

  @Get(':id')
  async getPokemonById(
    @Param(new ZodPipe(zGetPokemonByIdPath)) path: GetPokemonByIdData['path'],
  ) {
    const result = await this.handler.execute(new GetPokemonByIdQuery(path.id));
